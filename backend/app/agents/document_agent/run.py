"""
Document Agent Standalone Runner
=================================

Runs the Document Agent as a standalone A2A server.

Usage:
    python -m app.agents.document_agent.run
"""
...
from app.agents.document_agent.config.settings import get_settings
...
logger = logging.getLogger("DocumentAgentRunner")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description="A2A Server for Documentation Generation",
        version="1.0.0"
    )
    ...
    async def startup():
        ...
        logger.info(f"Document Agent started on port {settings.a2a_server_port}")
    ...
    async def create_task(request: TaskRequest):
        ...
        try:
            ...
            if request.task_type == "documentation_generation":
                result = await agent.handle_documentation_generation(task)
            elif request.task_type == "api_documentation":
                result = await agent.handle_api_documentation(task)
            else:
...
def main():
    ...
    logger.info(f"Starting Document Agent on port {port}")
